...
reset:			## resets the config files
	...
	rm -rf ./configs
	mkdir ./configs
	find ./configs_reference -type f -name "config*.dot" -exec cp -rp {} ./configs \;
	chmod -R 777 ./configs
	find ./configs_reference -type f -name "config*readonly.dot" -exec chmod a=r {} +
	find ./configs -type f -name "config*readonly.dot" -exec chmod a=r {} +

all: clean reset bin  ## all of the above

run: all              ## runs the project
	@printf '[\e[0;36mINFO\e[0m] Executing binary...\n'
	./$(ASSIGNMENT) configs/config_04.dot
